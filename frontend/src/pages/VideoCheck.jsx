...
import { Upload, Video, CheckCircle, AlertTriangle, Shield } from 'lucide-react';
...
const VideoCheck = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await client.post('/check_video', formData);
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-cyber-primary">Video Deepfake Detection</h1>
          <p className="text-slate-400">Analyze videos for AI-generated artifacts and manipulations</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="cyber-card md:col-span-2">
          <form onSubmit={handleSubmit} className="space-y-6" onDragEnter={handleDrag}>
            ...
            >
              ...
              <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center">
                ...
                <span className="text-lg font-medium text-slate-200">
                  {file ? file.name : 'Click to upload or drag video here'}
                </span>
            ...
            <button 
              type="submit" 
              className={`cyber-button w-full flex items-center justify-center space-x-2 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
        ...
        <div className="space-y-4">
          <div className="cyber-card bg-blue-500/5 border-blue-500/20">
            <h3 className="font-bold text-blue-400 mb-2 flex items-center"><Shield size={16} className="mr-2"/> How it works</h3>
            <p className="text-sm text-slate-400">Our model analyzes frame-by-frame inconsistencies and compression artifacts typical of deepfake algorithms.</p>
          </div>
        </div>
      </div>
